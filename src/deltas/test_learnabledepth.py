...
parser.add_argument('--seq_gap', default=7, type=int, help='gap between frames for ScanNet dataset')
...
def validate_with_gt(args, val_loader, supernet, trinet, depthnet, val_set=None):
    error_names = ['abs_rel', 'abs_diff', 'sq_rel', 'a1', 'a2', 'a3', 'MAE', 'RMSE', 'RMSE_log']
    ...
    with torch.no_grad():
        ...
        print(errors_depth)
        print(f"{error_names[0]}: {errors_depth.avg[0]}")
        print(f"{error_names[1]}: {errors_depth.avg[1]}")
        print(f"{error_names[2]}: {errors_depth.avg[2]}")
        print(f"{error_names[6]}: {errors_depth.avg[6]}")
        print(f"{error_names[7]}: {errors_depth.avg[7]}")
        print(f"{error_names[8]}: {errors_depth.avg[8]}")
