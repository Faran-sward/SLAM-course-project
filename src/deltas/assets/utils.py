...
class Scale(object):
    ...
    def __call__(self, images, depth, intrinsics, height=240, width=320):
        ...
        scaled_images = [cv2.resize(images[i], None, fx=x_scaling[i], fy=y_scaling[i], interpolation=cv2.INTER_LINEAR)
                         for i in range(len(images))]
        scaled_depth = [cv2.resize(depth[i], None, fx=dx_scaling[i], fy=dy_scaling[i], interpolation=cv2.INTER_LINEAR)
...
def compute_errors(gt, pred, valid, print_res=False):
    ...
    for current_gt, current_pred, current_valid in zip(gt, pred, valid):
        ...
        if len(valid_gt) == 0:
            ...
            # MAE
            mae += torch.mean(torch.abs(valid_gt - valid_pred))

            # RMSE
            rmse += torch.sqrt(torch.mean((valid_gt - valid_pred) ** 2))

            # RMSE log
            valid_gt_log = torch.log(valid_gt)
            valid_pred_log = torch.log(valid_pred)
            rmse_log += torch.sqrt(torch.mean((valid_gt_log - valid_pred_log) ** 2))

    if print_res:
        print(res_list)

    return [metric / batch_size for metric in [abs_rel, abs_diff, sq_rel, a1, a2, a3, mae, rmse, rmse_log]]
...
#         self.sum = [0]*i
#         self.count = 0
#         self.sum_squared_error = [0]*i
#         self.sum_absolute_error = [0]*i
#         self.sum_log_squared_error = [0]*i
...
#             self.sum[i] += v * n
#             self.avg[i] = self.sum[i] / self.count
#             self.sum_squared_error[i] += (v ** 2) * n
#             self.sum_absolute_error[i] += abs(v) * n
#             if v > 0:
#                 self.sum_log_squared_error[i] += (np.log(v) ** 2) * n

#     def rMSE(self):
#         return [np.sqrt(se / self.count) for se in self.sum_squared_error]

#     def MAE(self):
#         return [ae / self.count for ae in self.sum_absolute_error]

#     def rMSE_log(self):
#         return [np.sqrt(lse / self.count) for lse in self.sum_log_squared_error]

#     def __repr__(self):
#         val = ' '.join(['{:.{}f}'.format(v, self.precision) for v in self.val])
#         avg = ' '.join(['{:.{}f}'.format(a, self.precision) for a in self.avg])
#         rMSE = ' '.join(['{:.{}f}'.format(rmse, self.precision) for rmse in self.rMSE()])
#         MAE = ' '.join(['{:.{}f}'.format(mae, self.precision) for mae in self.MAE()])
#         rMSE_log = ' '.join(['{:.{}f}'.format(rmsel, self.precision) for rmsel in self.rMSE_log()])
#         return 'Value: {} (Avg: {}) | rMSE: {} | MAE: {} | rMSE_log: {}'.format(val, avg, rMSE, MAE, rMSE_log)
